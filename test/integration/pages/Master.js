				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("StudentNumber") === oObject.StudentNumber;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].StudentNumber;
										} else {
											sCurrentId = aItemsNotInTheList[0].StudentNumber;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("AdviseesListSet", {
											StudentNumber : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("StudentNumber"),
							title: oBindingContext.getProperty("StudentNumber")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "AdviseesListSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'AdviseesListSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("StudentNumber");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all AdviseesListSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "AdviseesListSet (" + iExpectedLength + ")"}),
									success : function () {
